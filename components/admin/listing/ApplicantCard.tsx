...
export default function ApplicantCard({
  ...
  // const gradYear = applicant.grad_year.split(' ').pop();

  return (
    <Card
      className={`${highlighted && "!bg-red-50 dark:!bg-red-300"} ${
        AdminTextStyles.card
      }`}
      onClick={() => {
        ...
        localStorage.setItem("selectedApplicantId", applicant.id);
      }}
    >
      <div className="flex flex-col items-center mb-1">
        ...
        <span className="text-sm text-gray-500 dark:text-gray-400">
          {`${applicant.major} | ${trueColleges.join(", ")} ${
            applicant.grad_year
          }`}
        </span>
      </div>
