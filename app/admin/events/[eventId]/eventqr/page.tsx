...
export default function EventQR({ params }: { params: { eventId: string } }) {
    const { token } = useAuth();
    const { mode } = useThemeMode();
    const [event, setEvent] = useState<EventMember | null>(null);
    ...
    if (!event) {
        return (
            <div className="space-y-6">
                ...
                <p className={DimmedAdminTextStyles.default}>Event not found.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
          ...
          <h1 className={AdminTextStyles.subtitle}>{event.name} - QR Code Check In</h1>
